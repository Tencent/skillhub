...
目前，SkillHub 已收录 **13 万+ Skills**，其中 **2 万+** 为企业发布的 Skills，并有 **500+** 商家完成上架。
...
如果你的产品预计有较大调用量，或会使用沙箱、模型等高消耗能力，建议在上线前联系我们。我们可以一起确认调用规模、配额和稳定性方案。详细说明见 [API 使用与资源支持](capabilities.md)。
...
| 能力 | 说明 | 文档 |
|------|------|------|
| Skill 检索 | 关键词、分类、来源、标签筛选，多维度排序与分页 | [skills.md](api/skills.md#skill-列表) |
| Skill 详情 | 单个详情与批量详情（单次最多 1000 个 slug） | [skills.md](api/skills.md#skill-详情) |
| 文件读取 | 某个版本的文件列表与单文件内容 | [files.md](api/files.md) |
| 版本与差异 | 版本记录、整体差异与单文件差异 | [versions-and-diff.md](api/versions-and-diff.md) |
| 质量结果 | 综合评分、评测维度与公开解释 | [evaluation.md](api/evaluation.md) |
| Skill 下载 | 按版本或 tag 下载 zip 包 | [skills.md](api/skills.md#下载-skill) |
| 分类与榜单 | 一级分类与 Top 排行榜 | [categories.md](api/categories.md) |

接口总览与通用约定见 [api/README.md](api/README.md)。
...
返回结构如下（完整字段说明见 [skills.md](api/skills.md#skill-列表)）：
...
```bash
# 在仓库根目录执行
bash docs/examples/curl/quickstart.sh          # 无额外依赖
node docs/examples/node/list-skills.mjs        # Node.js 18+
python3 docs/examples/python/list_skills.py    # Python 3.9+
```
...
联系信息和建议提供的材料见 [apply.md](apply.md)。
